...
class SCD30:
    ...
    def sendCommand(self, command, argument=None):
        if argument is None:
            ...
            argumentString = (chr(argumentMSB) +
                              chr(argumentLSB)).encode('utf-8')
    ...
    def read_n_bytes(self, n_bytes):
        ...
        if count == n_bytes:
            ...
            print("error: read measurement interval didnt return " +
                  str(n_bytes) + "B")
            return False
    ...
    def readMeasurement(self):
        ...
        struct_rH = struct.pack(
            '>BBBB', data[12], data[13], data[15],  data[16])
